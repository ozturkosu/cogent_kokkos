...
def tc_gen_code_Kernel( f,              name,               l_t3_d_decl_var,    l_t2_d_decl_var,    l_v2_d_decl_var,
    ...
    tc_code_kernel_head.tc_gen_code_Kokkos_Kernel_Head(f, name, l_t3_d_decl_var, l_t2_d_decl_var, l_v2_d_decl_var, l_input_strides, l_t3_idx, l_internal_idx, l_inputs_addr,
                                                opt_internal, opt_pre_computed, opt_data_type)
    ...
    tc_gen_code_Kokkos_Kernel_Initial(f,   size_sm_p7,         size_sm_a,      size_sm_b,
                                    l_t3_mapping_tb_2D, l_t3_idx,       l_t3_slices,
                                    size_reg_x,         size_reg_y,
                                    opt_gen_p7,         opt_gen_full,   opt_pre_computed,
                                    opt_shared_padding, opt_data_type,
    ...
    #   Step 5: Definition of shared memory part
    tc_code_kernel_SharedMemory(f, size_sm_p7,   size_sm_a,  size_sm_b)
...
def tc_code_kernel_SharedMemory(f, size_sm_p7,   size_sm_a,      size_sm_b):

    f.write("\t// For Shared Memory Initialization on Kokkos,\n")
    f.write("\tsize_t team_shmem_size(int team_size) const\n")
    f.write("\t{\n")
    f.write("\t\t return ( sharedViewTypeRight::shmem_size(" + str(size_sm_p7) + "," + str(size_sm_a) + ")" )
    f.write(" + sharedViewTypeRight::shmem_size(" + str(size_sm_p7) + "," + str(size_sm_b) + ") )\n" )
    f.write("\t}\n")
...
def tc_gen_code_Kokkos_Kernel_Initial(f,   size_sm_p7,         size_sm_a,      size_sm_b,          # For Shared memory
                                    l_t3_mapping_tb_2D, l_t3_idx,       l_t3_slices,        # For T3 (Output)
                                    size_reg_x,         size_reg_y,                         # For Register-Tiling
                                    opt_gen_p7,         opt_gen_full,   opt_pre_computed,   # Options
                                    opt_shared_padding, opt_data_type,                      # Options
    ...
    f.write("\n")
    f.write("\n")
    f.write("\tKOKKOS_INLINE_FUNCTION \n")
    f.write("\tvoid operator()(const team_member& thread) const{ \n")
    f.write("\n")
     
     
    #   Definition of team_size and team_rank
    f.write("\t\tint team_size = thread.team_size();\n")
    f.write("\t\tint team_rank = thread.league_rank();\n")
    
    f.write("\t\tint num_threads = SIZE_TB_1_Y;\n")
    f.write("\t\tint vector_lenght = SIZE_TB_1_X;\n")
    ...
    if opt_shared_padding == 1:
        ...
        f.write("\t\tsharedViewTypeRight sm_a(thread.team_scratch(0)," + str(size_sm_p7) + " , " + str(size_sm_a) + " + 1);\n")
        f.write("\t\tsharedViewTypeRight sm_b(thread.team_scratch(0)," + str(size_sm_p7) + " , " + str(size_sm_b) + " + 1);\n")
       
    else:
        #
       
        f.write("\t\tsharedViewTypeRight sm_a(thread.team_scratch(0)," + str(size_sm_p7) + " , " + str(size_sm_a) + ");\n")
        f.write("\t\tsharedViewTypeRight sm_a(thread.team_scratch(0)," + str(size_sm_p7) + " , " + str(size_sm_b) + ");\n")
    ...
    f.write("\t\t//Kokkos parallel_for loop definition \n")
    f.write("\t\tKokkos::parallel_for(Kokkos::TeamThreadRange(thread, num_threads), [&](const int& idx){\n")
    f.write("\n")

    f.write("\t\t\tint thread_rank = thread.team_rank() ; \n")
    f.write("\t\t\tint threadIdx_y = thread.team_rank; \n")
    f.write("\n")
    
    
    f.write("\t\t\tKokkos::parallel_for(Kokkos::ThreadVectorRange(thread, vector_lenght), \n")
    f.write("\t\t\t[&](const int& threadIdx){\n")
    f.write("\n")
    f.write("\n")
    
    f.write("\t\t\t\tint threadIdx_x = threadIdx;\n") 
    ...
    f.write("\t\t\t\tDataType temp_av;\n")
    if size_reg_y >= size_reg_x:
        f.write("\t\t\t\tDataType temp_bv["  + str(size_reg_y) + "];\n")                             # min(size_reg_y, size_reg_x), basically
        f.write("\t\t\t\tDataType reg_tile[" + str(size_reg_y) + "][" + str(size_reg_x) + "];\n")
    else:
        f.write("\t\t\t\tDataType temp_bv["  + str(size_reg_x) + "];\n")                             # min(size_reg_y, size_reg_x), basically
        f.write("\t\t\t\tDataType reg_tile[" + str(size_reg_y) + "][" + str(size_reg_x) + "];\n")
